...
public class UpdateHandlerStatusCommandHandler(
    ...
    : IRequestHandler<UpdateHandlerStatusCommand, Result>
{
    public async Task<Result> Handle(UpdateHandlerStatusCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!currentUserService.CanAccessAllRegions)
        {
            if (!handler.User.Regions.Any(r => currentUserService.HasAccessToRegion(r.Id)))
                return Result.Failure(Error.Forbidden("No permission to change the status of a handler in this region."));
        }

        if (handler.IsActive == request.IsActive)
            return Result.Failure(Error.Conflict($"Handler is already {(request.IsActive ? "active" : "deactivated")}."));
